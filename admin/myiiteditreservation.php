...
error_reporting(1);
  ...
  $user_txt = $_POST['txtuser'];
          ...
          buildingname=:buildingname,roomnumber=:roomnumber,approval=:approval,saleprice=:saleprice,reserve_date=:reserve_date, reserve_time=:reserve_time,reserve_endtime=:reserve_endtime, payment_type=:payment_type, payment_status=:payment_status, appointment_status=:appointment_status, user=:user where appointmentid=$id");
          ...
          if($update->execute()){
            ...
            $insert2=$pdo->prepare("insert into tbl_availability(bid,name,buildingname,roomnumber,date,start_time,end_time,description)
SELECT bid,name,buildingname,roomnumber,reserve_date,reserve_time,reserve_endtime,description FROM tbl_appointment where NOT EXISTS (SELECT bid,name,buildingname,roomnumber,date,start_time,end_time,description FROM tbl_availability where tbl_appointment.reserve_date = tbl_availability.date AND tbl_appointment.reserve_time = tbl_availability.start_time AND tbl_appointment.reserve_endtime = tbl_availability.end_time AND tbl_appointment.buildingname = tbl_availability.buildingname AND tbl_appointment.roomnumber = tbl_availability.roomnumber) AND approval = '$phrase3';");
      ...
      <!--------------------------
        ...
        <div class="box box-warning">
          <div class="box-header with-border">
            <h3 class="box-title"><a href="myiitreservationlist.php" class="btn btn-primary" role="button">Back to Reservation list</a></h3>
          </div>
