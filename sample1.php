...
if(!empty($name) AND !empty($buildingname) AND !empty($date) AND !empty($customerid) AND !empty($payment_type)){
  ...
  if($insert->execute()){
      ...
      $insert2=$pdo->prepare("insert into tbl_availability(bid,buildingname,roomnumber,date,start_time,end_time)
      values(:bid,:buildingname,:roomnumber,:date,:start_time,:end_time)");

      $insert2->bindParam(':bid',$bid);
      $insert2->bindParam(':buildingname',$buildingname);
      $insert2->bindParam(':roomnumber',$roomnumber);
      $insert2->bindParam(':date',$date);
      $insert2->bindParam(':start_time',$reserve_time);
      $insert2->bindParam(':end_time',$reserve_endtime);
